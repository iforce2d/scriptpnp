...
public:
	...
	NOTIFY_INLINE auto get_phase() -> const ImGuiToastPhase
	{
        ...
        if ((int)elapsed > NOTIFY_FADE_IN_OUT_TIME + this->dismiss_time + NOTIFY_FADE_IN_OUT_TIME)
		{
			return ImGuiToastPhase_Expired;
		}
        else if ((int)elapsed > NOTIFY_FADE_IN_OUT_TIME + this->dismiss_time)
		{
...
namespace ImGui
{
	...
	NOTIFY_INLINE void RenderNotifications()
	{
        ...
        float height = NOTIFY_PADDING_Y;

        int closedNotification = -1;
        ...
        for (auto i = 0; i < (int)notifications.size(); i++)
		{
            ...
            SetNextWindowPos(ImVec2(vp_size.x - NOTIFY_PADDING_X, height), ImGuiCond_Always, ImVec2(1.0f, 0.0f));
            PushStyleColor(ImGuiCol_Border, current_toast->get_color());
			...
			// Here we render the toast content
			{
                ...
                //PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.8f, 0.4f, 0.4f, 1.0f));
                //PushStyleColor(ImGuiCol_ButtonActive, ImVec4(0.9f, 0.3f, 0.3f, 1.0f));
                if ( Button( ICON_FA_WINDOW_CLOSE ) ) {
                    closedNotification = i;
                }
                //PopStyleColor(3);
        ...
        if ( closedNotification > -1 )
            RemoveNotification( closedNotification );
	}
