...
void TextEditor::Render(bool isFindDialogDoc)
{
	...
	if (!mLines.empty())
	{
		...
		while (lineNo <= lineMax)
		{
			...
			{
                ErrorMarker& em = errorIt->second;

                int bgColorInd = (int)( em.type == EMT_ERROR ? PaletteIndex::ErrorMarker : PaletteIndex::WarningMarker);

				auto end = ImVec2(lineStartScreenPos.x + contentSize.x + 2.0f * scrollX, lineStartScreenPos.y + mCharAdvance.y);
                drawList->AddRectFilled(start, end, mPalette[bgColorInd]);

                if ( windowHovered && ImGui::IsMouseHoveringRect(lineStartScreenPos, end))
                {
                    ImGui::PushFont(NULL);
					ImGui::BeginTooltip();
                    //ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 0.2f, 0.2f, 1.0f));
                    int titleColorInd = (int)PaletteIndex::ErrorMarkerTooltipTitle;
                    int detailsColorInd = (int)PaletteIndex::ErrorMarkerTooltipDetails;
                    if ( em.type == EMT_WARNING ) {
                        titleColorInd = (int)PaletteIndex::WarningMarkerTooltipTitle;
                    }
                    ImGui::PushStyleColor(ImGuiCol_Text, mPalette[titleColorInd]);                    
                    ImGui::Text("%s at line %d:", em.type == EMT_ERROR ? "Error":"Warning", errorIt->first);                    
					ImGui::PopStyleColor();
					ImGui::Separator();
                    ImGui::PushStyleColor(ImGuiCol_Text, mPalette[detailsColorInd]);
                    ImGui::Text("%s", em.msg.c_str());
					ImGui::PopStyleColor();
					ImGui::EndTooltip();
                    ImGui::PopFont();
				}
...
void TextEditor::Render(const char* aTitle, const ImVec2& aSize, bool aBorder, bool isFindDialogDoc)
{
    ...
    ImGui::BeginChild(
        ...
        aSize2,
        0,
        ImGuiWindowFlags_HorizontalScrollbar | /*ImGuiWindowFlags_AlwaysHorizontalScrollbar |*/ ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoNavInputs
    );
...
int TextEditor::GetTextLines(std::vector<std::string> &result) const
{
    result.reserve(mLines.size());
	...
	if (mCheckComments)
	{
		...
		for (auto currentCoord = Coordinates(0, 0); currentCoord < end; Advance(currentCoord))
		{
			...
			if (!line.empty())
			{
                ...
                if (currentCoord.mColumn == (int)line.size() - 1 && line[line.size() - 1].mChar == '\\')
					concatenate = true;
				...
				else
				{
					...
					else
					{
                        ...
                        else if (!withinSingleLineComment && startStr.size() > 0 && currentCoord.mColumn + startStr.size() <= line.size() &&
...
void TextEditor::EnsureCursorVisible()
{
    ...
    if (len + mTextStart < left + 40)
        ImGui::SetScrollX(std::max(0.0f, len + mTextStart - 40));
    if (len + mTextStart > right - 40)
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::PNPCommand()
{
    static bool inited = false;
    static LanguageDefinition langDef;
    if (!inited)
